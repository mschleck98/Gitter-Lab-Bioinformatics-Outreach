...
image_path = os.path.join(current_path, 'bioinfo')
...
def updateBoard(lives, WIDTH, HEIGHT,color, filename, myfont, screenInfo, screen):
    ...
    tmpsurface = myfont.render('PROTEIN PINBALL', False, (255,0,0))
    ...
    image = pygame.image.load(os.path.join(image_path, 'Alpha_Helix.png'))
    windowSurface.blit(image,(1000,25))
    image = pygame.image.load(os.path.join(image_path, 'download.png'))
    windowSurface.blit(image,(25, 600))
    image = pygame.image.load(os.path.join(image_path, 'amino.png'))
    windowSurface.blit(image,(950,650))
    tmp = myfont.render('Lives: ' + str(lives), False, (255,255,255))
    tmp_rect = tmp.get_rect()
    tmp_x = tmp_rect.width + 20
    tmp_y = tmp_rect.height + 20
    pygame.draw.rect(windowSurface, (255,0,0), [450, 680, tmp_x,tmp_y])
    windowSurface.blit(tmp,(460,690))
...
while True:
    ...
    image2 = pygame.image.load(os.path.join(image_path, 'maindq.png'))
    ...
    while newGame == False:
        ...
        windowSurface.blit(textsurface, (150,800))
        pygame.transform.scale(windowSurface, (screenInfo.current_w,screenInfo.current_h), tempSurface)
        screen.blit(tempSurface,(0,0))
        pygame.display.update()
        for event in pygame.event.get():
